...
class FritzAdvancedThermostat:
    ...
    def _load_raw_thermostat_data(self, force_reload=False):
        if not self._thermostat_data or force_reload:
            url = "/".join([self._prefixed_host, "data.lua"])
            req_data = {
                "sid": self._sid,
                "xhr": "1",
                "page": "sh_dev",
            ...
            while retries <= 3:
                try:
                    response = requests.post(
                        url,
                        headers=self._generate_headers(req_data),
                        data=req_data,
                        verify=self._ssl_verify, timeout=120)
                    break
                except ConnectionError as exc:
                    self._logger.warning(
                        "Connection Error on loading raw thermostat data")
                    retries += 1
                    if retries > 3:
            ...
            if response.status_code == 200:
                try:
                    raw_thermostat_data = json.loads(response.text)
                    if "devices" not in raw_thermostat_data["data"]:
                        err = "Error: Something went wrong loading the raw thermostat data\n" + response.text
                        self._logger.error(err)
                        raise FritzAdvancedThermostatExecutionError(err)
                except json.decoder.JSONDecodeError:
                    err = "Error: Didn't get a valid json response when loading the raw thermostat data\n" + response.text
                    self._logger.error(err)
                    raise FritzAdvancedThermostatExecutionError(err)
                self._thermostat_data = raw_thermostat_data
            else:
    ...
    def _set_thermostat_values(self, device_name, **kwargs):
        self._load_raw_thermostat_data()
        ...
        for dev in self._thermostat_data:
            ...
            while retries <= 3:
                try:
                    ...
                    self._logger.warning(f"Connection Error on setting thermostat: {dev}")
                    retries += 1
                    if retries > 3:
                        err = f"Tried 3 times, got Connection Error on setting thermostat: {dev}"
                        raise FritzAdvancedThermostatConnectionError(
                            err) from exc
    ...
    def set_thermostat_offset(self, device_name, offset):
        ...
        if not (float(offset) * 2).is_integer():
            ...
            self._logger.warning(
                f"Offset must be entered in 0.5 steps! Your offset was rounded to: {offset!s}")
        self._set_thermostat_values(device_name, Offset=str(offset))

    def get_thermostat_offset(self, device_name, force_reload=False):
        self._check_device_name(device_name)
        self._load_raw_thermostat_data(force_reload=force_reload)
    ...
    def get_thermostats(self):
        if not self._thermostats:
            for dev in self._devices.values():
                ...
                elif dev.productname in self._supported_thermostats:
                    self._thermostats.append(dev.name)
